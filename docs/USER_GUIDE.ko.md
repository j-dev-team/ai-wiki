# AI Wiki 1.1.4 사용자 설명서

이 문서는 Windows에서 AI Wiki 1.1.4를 설치하고 일반 위키와 목적별 위키를 한 대의 PC에서 운영하는 전체 절차를 설명합니다. AI Wiki는 로컬 단독 사용 도구이며 계정·로그인·서버 설정이 필요 없습니다. 웹 서버를 인터넷에 공개하지 말고, 고객 정보나 민감정보가 있는 위키는 운영체제 사용자 계정과 디스크 접근 권한으로 보호하세요. 전체 저장소 암호화와 접근 통제는 운영체제에서 관리합니다.
...
`migrate-schema`는 검사만 수행합니다. 1.1.4는 기존 v1·v2 문서를 읽을 때
